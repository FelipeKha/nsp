...
class TestTabuSearch:
    ...
    @pytest.fixture
    def tabu_search(self, get_population, get_neighbour, covering_cost):
        return TabuSearch(
            nb_nurses=4,
            nb_work_days_per_week=7,
            nb_shifts_per_work_day=1,
    ...
    def test_tabu_search_return_tuple_of_len_3(self, tabu_search):
        initial_solution = np.zeros((4, 7))
        out = tabu_search.tabu_search(initial_solution)
        assert len(out) == 3

    # first element is a numpy array
    def test_tabu_search_first_element_is_a_numpy_array(self, tabu_search):
        initial_solution = np.zeros((4, 7))
        out = tabu_search.tabu_search(initial_solution)
        assert type(out[0]) == np.ndarray

    # first element is a solution
    def test_tabu_search_first_element_is_a_solution(self, tabu_search):
        initial_solution = np.zeros((4, 7))
        out = tabu_search.tabu_search(initial_solution)
    ...
    def test_tabu_search_second_element_is_np_int64(self, tabu_search):
        initial_solution = np.zeros((4, 7))
        out = tabu_search.tabu_search(initial_solution)
    ...
    def test_covering_cost_less_than_or_equal_to_covering_cost_input_solution(
            self,
            tabu_search,
            covering_cost
    ):
        initial_solution = np.zeros((4, 7))
        initial_solution_cost = covering_cost.covering_cost(initial_solution)
        out = tabu_search.tabu_search(initial_solution)
        assert out[1] <= initial_solution_cost

    # third element is a list
    def test_tabu_search_third_element_is_a_list(self, tabu_search):
        initial_solution = np.zeros((4, 7))
        out = tabu_search.tabu_search(initial_solution)
        assert type(out[2]) == list

    # third element is a list of np.int64
    ...
    # search_solution
    # return tuple of len 3
    def test_search_solution_return_tuple_of_len_3(self, tabu_search):
        out = tabu_search.search_solution()
        assert len(out) == 3

    # first element is a numpy array
    def test_search_solution_first_element_is_a_numpy_array(self, tabu_search):
        out = tabu_search.search_solution()
        assert type(out[0]) == np.ndarray

    # first element is a solution
    def test_search_solution_first_element_is_a_solution(self, tabu_search):
        out = tabu_search.search_solution()
        assert self.is_solution(out[0])
    ...
    # second element is of type np.int64
    def test_search_solution_second_element_is_np_int64(self, tabu_search):
        out = tabu_search.search_solution()
        assert type(out[1]) == np.int64
    ...
    # third element is a list
    def test_search_solution_third_element_is_a_list(self, tabu_search):
        out = tabu_search.search_solution()
        assert type(out[2]) == list
