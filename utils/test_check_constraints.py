...
from utils.check_constraints import \
...
class TestCheckConstraints:
    ...
    # check_solution_for_max_work_days_per_week
    # returns a ndarray
    def test_check_solution_for_max_work_days_per_week_returns_ndarray(self, problem):
        solution = np.random.randint(0, 2, size=(4, 7))
        out = check_solution_for_max_work_days_per_week(solution, problem)
        assert type(out) == np.ndarray

    # returns a ndarray or dim (nb_nurses, nb_shifts)
    def test_check_solution_for_max_work_days_per_week_returns_ndarray_of_dim_nb_nurses_nb_shifts(self, problem):
        solution = np.random.randint(0, 2, size=(4, 7))
        out = check_solution_for_max_work_days_per_week(solution, problem)
        assert out.shape == (4, 7)

    # each item of output is either 0 or 1
    def test_check_solution_for_max_work_days_per_week_each_item_of_output_is_0_or_1(self, problem):
        solution = np.random.randint(0, 2, size=(4, 7))
        out = check_solution_for_max_work_days_per_week(solution, problem)
        assert np.array_equal(out, out.astype(bool))

    # each item of output is a solution
    def test_check_solution_for_max_work_days_per_week_each_item_of_output_is_a_solution(self, problem):
        solution = np.random.randint(0, 2, size=(4, 7))
        out = check_solution_for_max_work_days_per_week(solution, problem)
    ...
    def test_check_population_for_max_days_per_week_returns_ndarray(
        ...
        out = check_population_for_max_days_per_week(population, problem)
        assert type(out) == np.ndarray
    ...
    # each item of output is either 0 or 1
    def test_check_population_for_max_days_per_week_each_item_of_output_is_either_0_or_1(self, problem):
        population = np.random.randint(0, 2, size=(5, 4, 7))
        out = check_population_for_max_days_per_week(population, problem)
        for i in range(5):
            for j in range(4):
    ...
    # each item of output is a solution
    def test_check_population_for_max_days_per_week_each_item_of_output_is_a_solution(self, problem):
        population = np.random.randint(0, 2, size=(5, 4, 7))
        out = check_population_for_max_days_per_week(population, problem)
        for i in range(5):
            for j in range(4):
    ...
    # if input is only ones, will return complying solutions
    def test_check_population_for_max_days_per_week_if_input_is_only_ones_will_return_complying_solutions(
            self,
            problem
    ):
